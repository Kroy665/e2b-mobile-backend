...
The template is our own (`../opencode-template/opencode-sandbox`, sibling directory to this repo — a small `e2b` SDK-based template definition, not a raw Dockerfile), not a public one — built on `fromNodeImage('20')` with `opencode-ai` installed globally, so we control exactly what version ships rather than depending on a template we don't own staying current. Rebuild/republish with `npm run e2b:build:prod` from that directory after bumping the opencode version, then update `E2B_TEMPLATE_ID` here (`.env`, `.env.example`, `src/config/env.ts`'s default) to the new template id it prints.
...
- **Sandbox template version drift safety net**: `opencode`'s free tier occasionally requires a newer CLI version than whatever's baked into the template (this actually happened once — see the template note above for the real fix, rebuilding our own template). As a second line of defense against it happening again before anyone rebuilds the template, `createSandboxWithRepo` also runs a best-effort `opencode upgrade` once per sandbox right after cloning (non-fatal if it fails/times out, ~10-15s, only on create).
...
- **`GET /api/v1/sandboxes/:id/opencode/sessions/:sessionId`** — full message history for one session (`opencode export`), parsed into the same event vocabulary as the streaming/blocking endpoints (`text`, `tool_use`, `step_finish`, `error`) grouped by message, so history and live messages can share rendering code. Returns `{ sessionId, title, messages: [{ id, role, createdAt, events }] }`. `404` if the session id is validly-formed but doesn't exist in the sandbox; `400` if it doesn't even match the `ses_...` shape (caught by request validation before reaching the sandbox at all) — a client with a stale/corrupted stored session id should treat either as "clear it and start fresh," since which one a given bad id triggers isn't meaningfully distinguishable from outside.
